...
function ddlSubCategoryName() {
    try {
        $.post("/Machine/ddlSubCategoryName", {
...
function ddlCategoryName() {
    try {
        $.post("/Machine/ddlCategoryName", {}, function (data) {
            if (data.Result != "") {
...
function InsertUpdateAddProduct() {
    try {
        ...
        $.ajax({
            type: "POST",
            url: "/Machine/InsertUpdateAddProduct",
            ...
            success: function (data) {
                if (data.Status == "0") {
                    ...
                    ClearForm();
                    ShowAddProduct();
                    window.location.href = "/Machine/product";
                }
...
function ShowAddProduct() {
    try {
        $.post("/Machine/ShowAddProduct", {}, function (data) {
            if (data.Message != "") {
                alert(data.Message);
                ClearForm();
            }
...
function DelAddProduct(ProductID) {
    try {
        if (confirm("do you want to Delete?")) {
            $.post("/Machine/DelAddProduct", { ProductID: ProductID }, function (data) {
                if (data.Message != "") {
                    alert(data.Message);
                    ShowAddProduct();
                }
...
function EditAddProduct(PID) {
    ...
    try {
        $.post("/Machine/EditAddProduct", { ProductID: PID }, function (data) {
            if (data.Message != "") {
                alert(data.Message);
            }
            else {
...
function removeRow(element) {
    ...
    if (value != "0") {
        /*alert(value);*/
        $.post("/Machine/DeleteDescription", { id: value }, function (data) {
        });
    };
...
function addMore() {
    ...
    var newRow = '<div class="row" data-count="' + mainCount + '">' +
        '<div class="col-sm-12">' +
        '<input type="text" class="d-none" id="1id' + mainCount + '"  value="" hidden/>' +
        '<input type="text" class="form-control" id="txttitle' + mainCount + '" placeholder="Title" /><br />' +
        ...
        '</div>' +
        '</div>' +
        '<button class="remove text-decoration-none btn btn-danger" style="cursor:pointer;margin-top: 10px;"  onclick="removeRow(this)"><img src="../images/delete.gif" alt=""></button>' +
        '</div>' +
        '</div>';
...
function addFieldElement() {
    ...
    var newField = '<div class="row" data-count="' + count + '">' +
        ...
        '<div class="row">' +
        '<div class="col-sm-5">' +
        '<input type="text"class="d-none" value="" id="txtPdfId' + count + '"  hidden/>' +
        ...
        '</div>' +
        '</div>' +
        '<button class="remove btn btn-danger" style="margin-top: 10px;" onclick="removeFieldElement(this)"><img src="../images/delete.gif" alt=""></button>' +
        '</div>' +
        '</div>';
...
function removeFieldElement(element) {
    ...
    if (value != "0") {
        /*alert(value);*/
        $.post("/Machine/DeletePdf", { id: value }, function (data) {
        });
    }
    $(element).closest('.row').remove();
