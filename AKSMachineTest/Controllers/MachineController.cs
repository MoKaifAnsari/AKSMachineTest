...
using AKSMachineTest.Models;

namespace AKSMachineTest.Controllers
{
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 1; i < dt.Rows.Count; i++)
                    {
                        ...
                        sb.Append("<td><button type='button' class='btn btn-danger' onclick='DelAddSubProductCategory(" + dt.Rows[i]["AddSubCateId"].ToString() + ")'>Delete</i></td>");
                        sb.Append("<td><button type='button' class='btn btn-success' onclick='EditAddSubProductCategory(" + dt.Rows[i]["AddSubCateId"].ToString() + ")'>Edit</i></td></tr>");
                    }
                    sb.Append("</table>");
             ...
             {"@ProductID", "0" }
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        sb.Append("<td>" + dt.Rows[i]["Active"].ToString() + "</td>");
                        sb.Append("<td><button type='button' class='btn btn-danger' onclick='DelAddProduct(" + dt.Rows[i]["ProductID"].ToString() + ")'>Delete</td>");
                        sb.Append("<td><button type='button' class='btn btn-success' onclick='EditAddProduct(" + dt.Rows[i]["ProductID"].ToString() + ")'>Edit</td></tr>");
                    }
                    sb.Append("</table>");
                    dic["Grid"] = sb.ToString();
                }
